...
O layout — capa de ambientação, sumário, cabeçalho e rodapé institucionais, as cinco seções
numeradas e o quadro de aceite — vem de um template embutido, extraído de MIT044 reais já
aprovadas. Você escreve só o conteúdo; a formatação sai idêntica todas as vezes.
...
bullet), remove o miolo entre "Processo Atual" e "Aceite", esvazia a capa e grava
`template-mit044.docx` + `prototipos.xml`. Gere numa pasta temporária, confira a saída do script
...
assets/template-mit044.docx       esqueleto: capa, sumário, cabeçalho/rodapé, estilos
...
2. **Faça a alteração.** Se mexer no gerador, rode o validador contra um documento gerado antes e
   ...
   ```
   As 17 verificações precisam continuar passando. Se a sua mudança altera a estrutura de
